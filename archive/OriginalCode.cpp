...
long tempRef = 280;
// offset for temp correction calculation - 280 is about 30C for LM35 (set for value at normal room temperature)
...
void calculation() {
    ...
    TIC_ValueCorrOffset = TIC_Scaled * x1 + TIC_Scaled * TIC_Scaled * x2 / 1000.0 + TIC_Scaled * TIC_Scaled * TIC_Scaled
        * x3 / 1000000.0;

    TIC_Scaled = ((float)TIC_Value - TICmin) / (TICmax - TICmin) * 1000; // Scaling for TIC_Value
    TIC_ValueCorr = TIC_Scaled * x1 + TIC_Scaled * TIC_Scaled * x2 / 1000.0 + TIC_Scaled * TIC_Scaled * TIC_Scaled * x3
        / 1000000.0;
    ...
    if ( abs(diff_ns <6500) )// First check to avoid overflow in next calculation (also max VCO range is about 6500ns/s)
    {
        if (abs(diff_ns * gain) < (65535 + 200 * gain)) {
            TIC_ValueFiltered = TIC_ValueFiltered + (
                    (timer_us * 1000 + TIC_Value) * filterConst - TIC_ValueFiltered + (filterConst / 2))
                / filterConst;
        }
    }
    ...
    if (dacValue > (65535 * timeConst)) { dacValue = (65535 * timeConst); }

    dacValueOut = dacValueWithTempComp / timeConst; // PWM-DAC value
    if (dacValueOut < 0) { dacValueOut = 0; }
    if (dacValueOut > 65535) { dacValueOut = 65535; }

    // manual set of dacvalue if in hold and not 0, if zero hold old value
    if (holdValue > 0 && opMode == hold) { dacValueOut = holdValue; }
    ...
    if (i == 300) // 300sec
    {
        if (opMode == run) { StoreTIC_A[j] = sumTIC / i; }
        else { StoreTIC_A[j] = TIC_Value; }
        ...
        sumTemp = 0;
        if (opMode == run) { StoreDAC_A[j] = sumDAC / i; }
        else { StoreDAC_A[j] = (49999 - timer1CounterValue); }
...
float temperature_to_C(int RawADC, int sensor) {
    ...
    switch (sensor) {
        ...
        case 2: //10k NTC beta 3950 + 68k (15-60C)
            TempC = floatADC * floatADC * 0.0002536 - floatADC * 0.2158 + 88.48 - floatADC * floatADC * floatADC *
                0.0000001179;
            break;
        case 3: //10k NTC beta 3950 + 47k (15-65C)
            TempC = floatADC * floatADC * 0.0001564 - floatADC * 0.1734 + 92.72 - floatADC * floatADC * floatADC *
                0.00000005572;
            break;
        case 4: //10k NTC beta 3950 + 39k (25-70C)
            TempC = floatADC * floatADC * 0.0001667 - floatADC * 0.181 + 99.21 - floatADC * floatADC * floatADC *
                0.00000006085;
            break;
        case 5: //22k NTC beta 3950 + 120k (15-65C)
            TempC = floatADC * floatADC * 0.0001997 - floatADC * 0.1953 + 92.11 - floatADC * floatADC * floatADC *
                0.00000008010;
            break;
...
void getCommand() {
    ...
    if (Serial.available() > 0) {
        ...
        switch (ch) {
            ...
            case E:
                ...
                switch (z) {
                    case 1:
                        ...
                        for (int i = 0; i < 864; i++) { EEPROM.write(i, 0); }
                        EEPROM.write(1023, 0);
            ...
            case F:
                ...
                switch (z) {
                    case 1:
                        ...
                        Serial.println(
                            F("  gain = (65536/settable VCOrange in ppb) (eg. 100ppb DACrange gives gain=655)"));
                        Serial.println(F(
                        ...
                        Serial.println(F(
                            "Typing j<value><enter> Set temp sensor type 0=raw 1=LM35 2=10kNTC+68k 3=10kNTC+47k (second digit=adc1 eg 3x)"));
                        ...
                        Serial.println(
                            F("Typing t<value><enter> will set a new time constant between 4 and 32000 seconds"));
                        Serial.println(
                            F("Typing w<value><enter> will set a new warmup time between 2 and 1000 seconds"));
                        Serial.println("");
            ...
            case I:
                z = Serial.parseInt();
                if (z == 1) { nsDisplayedDecimals = !nsDisplayedDecimals; }
                else {
...
void printDataToSerial() {
    ...
    Serial.print("\t");
    if (temperature_Sensor_Type == 0) { Serial.print(tempADC2, DEC); }
    else {
    ...
    if (lessInfoDisplayed == false) {
        ...
        //Serial.print("\t");
        if (temperature_Sensor_Type / 10 == 0) { Serial.print(tempADC1, DEC); }
        else {
        ...
        if ((i >= 4) && (i <= 147)) {
            ...
            if (temperature_Sensor_Type == 0) { Serial.print(x, DEC); }
            else {
        ...
        if ((i >= 150) && (i <= 293)) {
            ...
            if ((x > 0) && (x < 65535)) {
                if (temperature_Sensor_Type == 0) { Serial.print(x % 10240, DEC); }
                else {
                    Serial.print(temperature_to_C((x % 10240) / 10, temperature_Sensor_Type % 10), 1);
                }
            }
            else { Serial.print(x, DEC); }
...
void setup() {
    ...
    TCCR1B |= ((1 << CS10) | (1 << CS11) | (1 << CS12) | (1 << ICES1) | (1 << ICNC1));
    // start Timer 1 and interrupt with noise cancel
...
void loop() {
    ...
    {
        // To detect missing pps the logic below is a bit difficult to understand.
        //
        // Period of 5MHz is 200ns. So every 200ns the 50.000 counter will count up.
        // So it takes 50.000 * 200ns = 10ms to overflow
        // So 100 times overlow is equal to 1 second.
        // Below we wait until we have at least 31 overflows.
        // But we also want overlow - 30 % 100 to be 0.
        // Given overlow must > 31 the subtraction start with 2.
        // This means we need at least 130 overflow counts before we enter the if statement.

        // This could just as well have been:
        // if (overflowCount >= 130 && (overflowCount - 130) % 100 == 0)
        ...
        if (overflowCount > 31 && (overflowCount - 30) % 100 == 0) // sense if more than 1sec since last pps pulse
        {
            ...
            delay(20);
            digitalWrite(ppsLockedLED, false);
            if (overflowCount > 2000) newMode = run;
            // resets timer_us etc after 20s without PPS in calculation function
